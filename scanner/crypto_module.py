...
تفحص CRYPTO_WATCHLIST (أعلى 30 عملة بالقيمة السوقية عبر بيانات Binance
...
from . import config, crypto_data
...
async def scan(cancel_event: asyncio.Event | None = None) -> list[dict]:
    """يفحص كل عملات CRYPTO_WATCHLIST، ويرجع أفضل CRYPTO_TOP_N نتيجة."""
    found: list[dict] = []
    ...
    return found[:config.CRYPTO_TOP_N]
