...
def generate_trading_tips(market: dict) -> list[str]:
    """نصائح تداول عامة **آلية مبنية على قواعد ثابتة** من حالة السوق
    الحالية (المزاج، مستوى VIX) -- جزء من صفحة "ملخص السوق اليومي" بلوحة
    الويب. عامة بطبيعتها (لا تخص سهماً أو عقداً محدداً)، وليست ذكاءً
    اصطناعياً توليدياً ولا توصية استثمارية مباشرة."""
    mood = market.get("mood", "غير متاح") if market else "غير متاح"
    vix = market.get("vix") if market else None
    tips: list[str] = [
        "لا تخاطر بأكثر مما تتحمّل خسارته على مركز واحد -- حجم المركز أهم من دقة التوقيت.",
    ]

    if mood in ("متفائل", "متفائل بحذر"):
        tips.append("الاتجاه العام صاعد، لكن هذا لا يلغي المخاطر الفردية -- تحقّق من احتمالية "
                    "الربح والتقلب الضمني لكل عقد بمفرده قبل الدخول.")
        tips.append("في الأسواق الصاعدة يسهل ملاحقة الأسهم بعد ارتفاعها (FOMO) -- التزم "
                    "بشروطك المعتادة بدل الانجراف خلف الزخم.")
    elif "خوف" in mood:
        tips.append("تقلب مرتفع يعني بريميوم أغلى وحركة سعرية أعرض -- قلّل حجم المراكز "
                    "الجديدة وراقب اتساع السبريد قبل أي تنفيذ.")
        tips.append("في بيئة الخوف المرتفع، خطة الخروج (وقف خسارة مسبق) أهم من أي وقت -- لا "
                    "تنتظر \"التعافي\" بلا حد أقصى للخسارة المقبولة.")
    elif mood != "غير متاح":
        tips.append("سوق متوازن بلا اتجاه واضح -- الانتقائية أهم من الكمية، ركّز على العقود "
                    "الأعلى تصنيفاً (🥇/🥈) بدل محاولة تغطية أكبر عدد ممكن.")

    if vix is not None and vix >= 20:
        tips.append(f"مؤشر VIX عند {vix:.1f} -- توقّع سبريدات أوسع وتنفيذاً أبطأ من المعتاد، "
                    "تأكد من السعر الفعلي المعروض قبل التنفيذ لا السعر النظري فقط.")

    tips.append("هذه نصائح عامة وليست توصية لأي سهم أو عقد بعينه -- كل قرار تداول مسؤوليتك "
               "الكاملة.")
    return tips
